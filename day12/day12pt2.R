  ...
  if (counts[row,col] > 0) {
    ...
    if (is.na(lower_case)) {
      ...
      col <- col + 1
        ...
        } else if (any(dfo[row,] == as.character(paths[as.integer(counts[row,col]),2])) == TRUE) {
          if (is.na(sum(dfo[row,] == as.character(paths[as.integer(counts[row,col]),2])))) {
             lower_flg <- lower_flg
           } else if (sum(dfo[row,] == as.character(paths[as.integer(counts[row,col]),2])) > 1) {
             lower_flg <- as.character(paths[as.integer(counts[row,col]),2])
           } else {
             lower_flg <- lower_flg
           }
          if (lower_flg == "?") {
