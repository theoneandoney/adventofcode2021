...
#input_loc <- "./day15/day15input.txt"
input_loc <- "./day15/day15example.txt"

dat <- data.table::fread(input_loc, header = FALSE)
df <- as_tibble(str_split_fixed(dat$V1, "", n = Inf), .name_repair = "unique") |> mutate(across(everything(), as.integer))
mat <- as.matrix(df)
